...
### Structure
* 3 parties séparés par un point : header.payload.signature

#### headers
* L'en-tête se compose de deux parties : le type et l'algorithmede hashage 
...
```
iss : celui qui a créé le token
exp: heure expiratioN
...
* claime privées: claims personnalisés, ne rentre pas dans l'interopérabilité mais ne concerne que mes applications
...
* Le token est souvent transmis avec un prefixe au serveur : Authoriation: Bearer H.P.S - Bearer = Porteur
...
* protèction XSS : utiliser le flag HttpOnly (JavaScript n'a pas le droit de lire ce cookie) et Secure (ne peut être envoyer que par https)
* expose aux attaques CSRF

### comment protéger CSRF
...
* Confirmation des emails :
	...
	* JWT
		...
		* on génère un lin avec le token (durée de vie limitéé)
